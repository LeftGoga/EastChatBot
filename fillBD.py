...
if __name__ == '__main__':
    ...
    print(all_data["_embedded"]["items"])
    for j in all_data["_embedded"]["items"]:
        pattern = r"(\d{4})-(\d{2})"
        matches = re.findall(pattern, j["name"])
        stream = test.get_file(j["name"])
        print(matches)
        if matches:
            ...
            if record.rec_type == 'response':
                ...
                if 'text/html' in ct:  # это html-ка
                    ...
                    if status == '200 OK':  # скачано нормально
                        ...
                        ids=text_atr['id']
                        data=[str(text_atr['text'])]#Данные в базу данных
                        metadatas = [{'source':str(text_atr['link'])}]

                        db.add_to_collection(paper["paper_year"],ids,data,metadatas)
